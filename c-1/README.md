...
- データに対して実行するクエリを用意する
    ...
    -
=======
- mysqldumpはデフォルトでCREATE T## 1.6 MySQLの歴史

**バージョン3.23(2001年)**

- MySQLがブレークした
- それまではフラットファイル上のクエリ言語
- ISAMに代わるストレージエンジンとしてMyISAMが導入
- InnoDBも提供されていたが、標準バイナリディストリビューションには含まれていなかった
- 全文インデックス機能とレプリケーション機能が追加

**バージョン4.0(2003年)**

- UNIONやマルチテーブルDELETE文のサポートなど、新しい構文機能が追加された
- これまでは1つのスレッドですべての作業を行っていたので、タスクの切り替えに苦労した
- 1つのレプリカで2つのスレッドを使用するように書き換えられた
- InnoDBはサーバーの標準パーツとして提供される
    - 行レベルのロックや外部キーといった機能を揃えた
- これ移行クエリキャッシュ機能は変更されていない
- SSL接続もサポート

**バージョン4.1(2005年)**

- サブクエリやINESRT ON DUPLICATE KEY UPDATEなどクエリ構文機能がさらに追加される
- UTF-8文字がサポート
- バイナリプロトコルとプリペアドステートメントのサポートが新たに追加される

**バージョン5.0(2006年)**

- ビュー、トリガ、ストアドプロシージャ、ストアドファンクションなどのエンタープライズ機能が追加
- ISAMエンジンが削除
- Federatedなどの新しいストレージエンジンが導入

**バージョン5.1(2008年)**

- パーティショニングと行ベースのレプリケーション
- プラグイン可能なストレージエンジンAPIなどのプラグインAPI
- BerkeleyDBやFederatedなどのストレージエンジンも使用されなくなる

**バージョン5.5(2010年)**

- パフォーマンス、スケーラビリティ、レプリケーション、パーティショニング、Windowsのサポートにフォーカス
- InnoDBがデフォルトのストレージエンジンになる
- インストルメンテーション（計測機能）の最初の改良とともにPERFORMANCE_SCHEMAデータベースが追加
- レプリケーション、認証、監査のための新しいプラグインAPIも追加
- 半同期レプリケーション用のプラグインが利用可能になる
- 認証とスレッドプール用の商用プラグインがOracleによってリリース
- InnoDBのアーキテクチャにバッファプール分割機能

**バージョン5.6(2013年)**

- クエリオプティマイザの見直し
- 全文テキスト検索を対象とした追加のプラグインAPI
- レプリケーション改善
- PERFORMANCE_SCHMEAデータベースのインストルメンテーションの大幅な強化といった新機能が追加

**バージョン6.0(中止)**

- バージョン5.5または5.6でリリースされている
- すべてのストレージエンジンでのオンラインバックアップやサーバーレベルの外部キー、サブクエリの改良、スレッドプールなどの新機能が追加されるはずだった

要約

- ライフサイクルの早い段階では破壊的イノベーションであった
    - 魅力的な機能と価格でキラーアプリケーションになる
- 初期の5.xリリースではビューやストアドプロシージャといった機能でエンタープライズへの進出を図るが、バグと脆弱さに悩まされる
- MySQL5.5は最も高品質なリリース

パフォーマンス

- 並行性が高い状態ではInnoDBプラグインが含まれているMySQLのバージョンのほうがずっとパフォーマンスが高い
    - よりスケーラブル
- MySQLの最新のバージョンはシングルスレッドのワークロードでは古いバージョンよりもパフォーマンスが悪い
    - 最近のサーバーでは以前よりも複雑なSQL文法がサポート
    - 多くの機能や改良点が追加されている
    - 単純なクエリでは無駄なオーバーヘッドが増える
- どのバージョンを使用するかは技術的なニーズよりもビジネスニーズで決まるABLEコマンドの前にDROP TABLEコマンドを書き出すため、注意していないとデータを失う

## 1.7 MySQLの開発モデル

- 次のGAリリースに含まれる予定のプレビューと併せて、新しい開発マイルストーンを定期的にリリースしている
- 準備が整ったと判断した機能をバンドルし、サーバーの新しいGAリリースとして配布している
- MySQLはGPLライセンスでオープンソース
- オープンコアモデルに従っている
- クリップルウェア開発モデルを表すものではない
    - サーバーは拡張なしでも十分すぎる

## 1.8 まとめ

- MySQLは階層型のアーキテクチャを採用している
    - 上層ではサーバーレベルのサービスとクエリの実行がサポートされる
    - 下層ではストレージエンジンが提供される
- 最も重要なのはストレージエンジンAPI
- MySQLがストレージエンジンAPIを通じて行をやりとりすることでクエリを実行することを理解していれば、サーバーアーキテクチャの基本原理の1つを理解できている
- MySQLはISAMと後継のMyISAMをベースに開発された
- InnoDBにはALTERがトランザクションを実行しないようにする仕掛けがない
- InnoDBによって実行されるものはすべてトランザクション対応
- ストレージエンジンAPIの欠点
    - 選択肢が必ずしもよくない
    - MySQL5.0/5.1の時代にストレージエンジンが次々登場した結果、選択肢が増えすぎてしまった
    - 95%以上のユーザーにとってはInnoDBが非常によいストレージエンジンである
- OracleがInnoDBとMySQLを傘下に修めた
    - 引き継ぎ気GPLライセンスとして配布され、オープンソースのまま
>>>>>>> e8d8b9e9df8b4b20480f84228fbf6daa5862de35
